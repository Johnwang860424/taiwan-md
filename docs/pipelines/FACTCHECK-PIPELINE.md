...
## 跟 REWRITE-PIPELINE 的整合

REWRITE Stage 3.5/3.6 內嵌「快速版」FACTCHECK：

- 預算 10-30 min
- 只跑 Phase 2-4 採樣
- 主 session 自跑

本 pipeline（FACTCHECK）是「**完整版**」，觸發於：

- 觀察者要求（「做完整嚴謹的事實查核」）
- 月度巡邏
- 外部 PR 進入主分支前的接收層 audit
- 讀者公開質疑某 atom 後的擴大調查

**REWRITE 的 Stage 3.5/3.6 hard gate 不變**——本 pipeline 是更深一層的 hardening，不取代 REWRITE 內建 gate。
...
_relations: REWRITE-PIPELINE.md §Stage 3.5/3.6（內嵌快速版） / RESEARCH.md（從 0 蒐集 vs 本檔從 1 反查） / EDITORIAL.md §挖引語制度 / MANIFESTO §10 幻覺鐵律 / DNA #16/#22/#23/#26 / MAINTAINER-PIPELINE §Footnote source authority audit_
